...
function createBingoCards() {
  const blockTexts = boards.trim().split('\n\n');
  return blockTexts.map((blockText) => {
    const lines = blockText.trim().split('\n');
    return lines.map((line) => line.trim().split(/\s+/).map(Number));
  });
}

function markBingoCard(card, markedNumbers) {
...
function isNumberMarked(card, number) {
  for (let row = 0; row < card.length; row++) {
    for (let col = 0; col < card[row].length; col++) {
...
function checkWin(card) {
  const numRows = card.length;
  const numCols = card[0].length;

  for (let row = 0; row < numRows; row++) {
    if (card[row].every(cell => cell === 'X')) {
      return true;
    }
  }

  for (let col = 0; col < numCols; col++) {
    let isVerticalWin = true;
    for (let row = 0; row < numRows; row++) {
      if (card[row][col] !== 'X') {
        isVerticalWin = false;
        break;
...
function calculateRemainingSum(card, calledNumbers) {
  let sum = 0;
  for (let row = 0; row < card.length; row++) {
    for (let col = 0; col < card[row].length; col++) {
      const cellValue = card[row][col];
      if (typeof cellValue === 'number' && !calledNumbers.includes(cellValue)) {
        sum += cellValue;
      }
    }
  }
